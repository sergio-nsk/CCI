...
// whether is a route between to nodes.
...
// Width visiting
bool routeBetwenNodes(const Graph<State>& graph,
                      const Node<State>& from,
                      const Node<State>& to) {
  if (from == to)
    return true;
...
bool routeBetwenNodesWalker(const Graph<State>& graph,
                            const Node<State>& from,
                            const Node<State>& to) {
  if (from == to)
    return true;
...
bool routeBetwenNodesR(const Graph<State>& graph,
                       const Node<State>& from,
                       const Node<State>& to) {
  for (auto& n : graph.getNodes())
    n->state = Unvisited;
...
bool test(const Graph<State>& graph, size_t from, size_t to) {
  ...
  std::cout << toNode->Name() << " is " << (result ? "" : "NOT ")
            << "reachable from " << fromNode->Name() << std::endl;
  std::cout << toNode->Name() << " is " << (resultR ? "" : "NOT ")
            << "reachable from " << fromNode->Name() << std::endl;
  return result;
}

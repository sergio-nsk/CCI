...
template <typename T>
bool compareTrees(const Node<T>* left, const Node<T>* right);

template <typename T>
bool checkSubtree(const Node<T>* root, const Node<T>* subtreeRoot);
...
bool compareTrees(const Node<T>* left, const Node<T>* right) {
  if (!left && !right)
    return true;
           ...
           !right)  // one tree is not finished, another already finished
    return false;
...
bool checkSubtree(const Node<T>* root, const Node<T>* subtreeRoot) {
  if (!root)
    return false;
  if (root->getValue() == subtreeRoot->getValue() &&
      compareTrees<T>(root, subtreeRoot))
    return true;
  return checkSubtree<T>(root->getLeft(), subtreeRoot) ||
         checkSubtree<T>(root->getRight(), subtreeRoot);
}

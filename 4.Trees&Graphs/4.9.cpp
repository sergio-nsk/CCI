...
template <typename T>
std::list<std::deque<T>> possibleStableJoins(std::deque<T>& prefix,
                                             std::deque<T>& left,
                                             std::deque<T>& right);

template <typename T>
std::list<std::deque<T>> possibleArrays(const Node<T>* root) {
  std::list<std::deque<T>> result;
  if (!root) {
    result
        .emplace_back();  // empty is required to call cycle body at least once
    return result;
  }

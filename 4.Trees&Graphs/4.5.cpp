...
template <typename T>
bool checkNode(const Node<T>* node, const T* minValue, const T* maxValue) {
  if (!node)
    return true;
...
int main() {
  ...
  std::cout << "The tree is " << (isValidBST<int>(tree) ? "" : "NOT ")
            << "binary search tree" << std::endl;

  // invalid BST
  tree.getRoot()->getRight()->getRight()->getRight()->setLeft(
      std::make_unique<Node<int>>(19));
  TestUtils::printTree(tree);
  std::cout << "The tree is " << (isValidBST<int>(tree) ? "" : "NOT ")
            << "binary search tree" << std::endl;

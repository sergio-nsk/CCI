...
// My solution is faster than one from the book. commonAncestor() or
// commonAncHelper() function from the book always recursively traverse subtrees
...
template <typename T>
FindResult findNodeFrom(const Node<T>* startNode, const Node<T>* node) {
  if (!startNode)
    return NotFound;
  if (startNode == node)
    return FoundEqual;
...
template <typename T>
const Node<T>* findCommonAncestor(const Tree<T>& tree,
                                  const Node<T>* one,
                                  const Node<T>* two) {
  if (one == two)
    return one;
  ...
  while (firstResult == secondResult) {
    startNode = (firstResult == FoundLeft) ? startNode->getLeft()
                                           : startNode->getRight();
...
int main() {
  auto tree = TestUtils::treeFromArray({10, 1,  12, 3,  14, 25, 16, 27, 18, 29,
                                        10, 13, 2,  15, 4,  5,  17, 7,  19, 9});
  TestUtils::printTree(tree);

  auto i = begin(tree);

  for (const auto& one : tree) {
    if (one == tree.getRoot())
    ...
    for (auto two : tree) {
      ...
      std::cout << (ancestor ? std::to_string(ancestor->getValue())
                             : "NOT FOUND")
                << std::endl;
    }
  }
  ...
  std::cout << (ancestor ? std::to_string(ancestor->getValue()) : "NOT FOUND")
            << std::endl;
}

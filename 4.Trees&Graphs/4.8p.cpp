...
template <typename T>
const Node<T, true>* findCommonAncestor(const Tree<T, true>& tree,
                                        const Node<T, true>* one,
                                        const Node<T, true>* two) {
  if (one == two)
    return one;
  ...
  if (depthL > depthR) {
    ...
    std::swap(one,
              two);  // that is why we pass 'one' and 'two' arguments by values
  }
...
int main() {
  auto tree = TestUtils::treeFromArray<int, true>({10, 1,  12, 3,  14, 25, 16,
                                                   27, 18, 29, 10, 13, 2,  15,
                                                   4,  5,  17, 7,  19, 9});

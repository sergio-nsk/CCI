...
template <typename T>
int getHeight(const Node<int>* node) {
  if (!node)
    return 0;
...
int main() {
  ...
  std::cout << "Tree is " << (isTreeBalanced<int>(tree) ? "" : "NOT ")
            << "balanced" << std::endl;

  // Disconnect right subtree, the result tree is not balanced
  auto r = tree.getRoot()->extractRight();
  tree.getRoot()->setRight(std::make_unique<Node<int>>(100));
  std::cout << "Tree is " << (isTreeBalanced<int>(tree) ? "" : "NOT ")
            << "balanced" << std::endl;

  // Return right subtree and check if the tree is balanced again
  tree.getRoot()->setRight(std::move(r));  // balanced
  std::cout << "Tree is " << (isTreeBalanced<int>(tree) ? "" : "NOT ")
            << "balanced" << std::endl;
  return 0;
}

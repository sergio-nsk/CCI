...
// Tree contains all nodes in a deque. It needs to handle insert() and delete()
 ...
 public:
  ...
  class RandomNode {
   ...
   private:
    ...
    T v;
    size_t idx;  // index in the Tree::nodes, used for quick deletion from
                 // Tree::nodes
    friend class Tree;
  };
  ...
  const NodePtr insert(const NodePtr& parent, U&& value, bool insertRight) {
    ...
    if (!parent)
      ...
      parent->childs[insertRight ? 1 : 0] = newNode =
          std::make_shared<RandomNode>(std::forward<U>(value));
  ...
  const NodePtr getRundomNode() const {
    if (nodes.size() == 0)
      ...
      if (randomDistribution.max() != nodes.size() - 1)
        randomDistribution =
            std::uniform_int_distribution<size_t>(0, nodes.size() - 1);
      return nodes.at(randomDistribution(randomEngine));
    }
  }

  const NodePtr& root() const { return this->rootNode; }

...
Person searchLevel(const std::unordered_map<int, Person>& people,
                   BFSData& primary,
                   BFSData& secondary);

std::deque<Person> findPathBiBFS(const std::unordered_map<int, Person>& people,
                                 int source,
                                 int destination) {
...
Person searchLevel(const std::unordered_map<int, Person>& people,
  ...
  size_t count = primary.toVisit.size();
...
std::deque<Person> mergePaths(BFSData& bfs1, BFSData& bfs2, int connection) {
  ...
  pathOne.insert(pathOne.end(), pathTwo.begin(),
                 pathTwo.end());  // add second path
  return pathOne;
}
...
int main() {
  ...
  const int edges[][2] = {{1, 4}, {1, 2},  {1, 3},  {3, 2}, {4, 6}, {3, 7},
                          {6, 9}, {9, 10}, {5, 10}, {2, 5}, {3, 7}};

// Stack of Boxes: You have a stack of n boxes, with widths w1 , heights h i, and depths d i . The boxes
...
int createStack(const std::deque<Box> &boxes, const Box *bottom, unsigned offset, std::vector<int> &map);

int createStack(std::deque<Box> &boxes)
{
    // Sort in decending order by height.
    std::sort(boxes.begin(), boxes.end(), [](const Box &left, const Box &right) {return left.height < right.height;});

...
// digit. The digits are stored in reverse order, such that the 1st digit is at
...
template <typename T>
bool sumTail(const typename forward_list<T>::Node* left,
             const typename forward_list<T>::Node* right,
             forward_list<T>& result) {

...
class forward_list {
 public:
  ...
  forward_list(forward_list&& rhv)
      : head(std::move(rhv.head)), len(std::move(rhv.len)) {
    rhv.head = nullptr;
    rhv.len = 0;
  }

  forward_list(std::initializer_list<T> initVals) noexcept : forward_list() {
    for (auto& v : initVals)
      push_back(v);
  }

  forward_list<T>& operator=(forward_list<T>&& r) noexcept {
    if (this == &r)
      return *this;

    clear();
    std::swap(this->head, r.head);
    std::swap(this->len, r.len);
    return *this;
  }
 ...
 public:
  class iterator {
   public:
    ...
    bool operator==(const iterator& r) { return ptr == r.ptr; }
  ...
  const Node* headNode() const { return head; }

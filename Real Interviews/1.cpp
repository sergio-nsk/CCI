// A data source generates a continueuse stream of data packets of varying
// length, These are to be passed through a processor that requires fixed-length
// blocks. The data is in the form of a sequence of complex numbers encoded as a
// pair of float values. Write a class that accepts variable length packets,
// generates fixed length packets and passes these to an internal 'process'
// method. No assumptions should be made about the sizes or relative sizes of
// the packets to be processed or the size of packets arriving.
...
class RundomGenerator {
 public:
  static void init() {
    ...
    randomGenerator = std::uniform_real_distribution<float>();  // Let [0, 1)
                                                                // floats by
                                                                // default
  }
  ...
  static size_t fillPacket(ComplexNumber* buffer, size_t N) {
    size_t count = std::uniform_int_distribution<size_t>(
        1, N)(randomEngine);  // Let packet size from 1 till N randomly
...
class Transmitter {
 public:
  Transmitter(Processor& p) : process(p) { outBuffer.reserve(N); }
...
int main() {
  ...
  std::cout << "Received " << ReceivedNumberCount << " complex numbers"
            << std::endl;

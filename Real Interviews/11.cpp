...
class Adapter {
 public:
  Adapter(const std::vector<Chunk>& vec) : _vec(vec) { reset(); }
...
int innerProductNaive(const std::vector<Chunk>& vec1,
                      const std::vector<Chunk>& vec2) {
  int product = 0;
  Adapter a1(vec1);
  Adapter a2(vec2);
...
// Optimal
int innerProduct(const std::vector<Chunk>& vec1,
                 const std::vector<Chunk>& vec2) {

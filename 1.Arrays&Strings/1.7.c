...
// Returns pointer to matrix element at row and col position (r, c)
int* at(int* matrix, int n, int r, int c) {
  return &matrix[r * n + c];
}

// Returns pointer to destination where element (r, c) should be after rotation
int* rightAt(int* matrix, int n, int r, int c) {
  return &matrix[c * n + (n - r - 1)];
}

// Returns pointer to destination element where element (r, c) would be after
// two rotations
...
// Returns pointer to destination element where element (r, c) would be after
// three rotations or one left rotation
